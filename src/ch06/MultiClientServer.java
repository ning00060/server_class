...
public class MultiClientServer {
	...
	public static void main(String[] args) {
		...
		try (ServerSocket serverSocket = new ServerSocket(PORT)) {
			...
			while (true) {
				// 1. serverSocket.accpet()호출하면 블로킹 상태가 된다.멈춰있음
				// 2. 클라이언트가 욘결 요청하면 새로운 소캣 객체 생성이 된다.
				// 3. 새로운스레드를 만들어 처리 ...(클라이언트가 데이터를 주고 받기 위한 스레드)
	...
	private static class ClientHandler extends Thread {
		...
		public void run() {
			try {
				...
				// 여기서 중요 ! - 서버가 관리하는 자료 구조에 자원 저장(클,연결된 소켓->outStream)
				...
				// 받은 데이터를 서브측과 연결된 클라이언트에게 데이터를 전달하자.
